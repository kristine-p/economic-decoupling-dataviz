...
# thermal gradient for climate-stress choropleths — deliberately distinct
# from both the Tapio and PM2.5 palettes used elsewhere in the app.
THERMAL_SCALE = [
    [0.00, "#1B3A4B"],
    [0.30, "#2C6E7F"],
    [0.55, "#E8C24A"],
...
def _add_choropleth(fig, df, metric_col, row=None, col=None):
    vals = df[metric_col]
    fig.add_trace(
        go.Choropleth(
            locations=df["iso3"],
            z=vals,
            zmin=0, zmax=max(vals.max(), 1) if len(vals) else 1,
            ...
            customdata=df[["country", metric_col]].values,
...
def build_single_map(df: pd.DataFrame, metric_col: str) -> go.Figure:
    fig = go.Figure()
    _add_choropleth(fig, df, metric_col)
    _style_geo(fig)
...
def build_compare_map(df_left: pd.DataFrame, df_right: pd.DataFrame, metric_col: str,
                       label_left: str, label_right: str) -> go.Figure:
    fig = make_subplots(
        rows=1, cols=2,
    ...
    # shared color range across both panels for a fair visual comparison
    zmax = max(df_left[metric_col].max() if len(df_left) else 1,
               df_right[metric_col].max() if len(df_right) else 1, 1)
...
def build_trend_chart(climate_df: pd.DataFrame, iso3: str, metric_col: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="rgba(15,23,31,0.08)", tickfont=dict(size=9)),
        hoverlabel=dict(bgcolor=COLORS["panel_solid"], bordercolor=COLORS["hairline"],
